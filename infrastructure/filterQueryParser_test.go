...
func TestParseFilterParamsWithOneParam(t *testing.T) {
	...
	if len(params["status"]) < 1 {
		t.Fatalf("expected status param with value 'active' but status is not present")
	}
...
func TestParseFilterParamsWithEmptyQuery(t *testing.T) {
	...
	params, err := ParseFilterParams(query)
	if err != nil {
		t.Fatalf("error parsing query filter %s", err)
	}

	if len(params) > 0 {
		t.Fatal("expected empty param list")
	}
}
...
func TestParseFilterParamsWithMoreThanOne(t *testing.T) {
	...
	if len(params["status"]) < 1 {
		t.Fatal("expected status param with value 'active' but status is not present")
	}
